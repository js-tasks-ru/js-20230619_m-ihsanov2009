...
export default class ProductForm extends BaseClass {
  ...
  createListeners() {
    ...
    this.subElements.imageListContainer.addEventListener(
      "pointerdown",
      this.onImageListContainerClick,
      {
        signal: this.controller.signal,
      }
    );
  ...
  getTemplate() {
    return `<div class="product-form">
        ...
        ${this.generateDescriptionTemplate()}
        ${this.generateImagesTemplate()}
        ${this.generateCategoriesTemplate()}
        ${this.generatePropertiesTemplate()}
        ${this.generateButtonsTemplate()}
        </form>
      </div>`;
  }
  ...
  generateCategoriesTemplate() {
    return `<div class="form-group form-group__half_left">
      ...
      <select class="form-control" name="subcategory" id="subcategory">
        ${this.categories
          .map((cat) => {
            return this.generateCategoryTemplate(cat);
          })
          .join("")}
      </select>
    </div>`;
  ...
  generateCategoryTemplate(cat) {
    return cat.subcategories
      .map((item) => {
        return this.generateSubcategoryTemplate(cat.title, item);
      })
      .join("");
  }

  generateSubcategoryTemplate(parentName, item) {
